...
const Home: React.FC = () => {
  ...
  const getAllUsers = async () => {
    try {
      setLoading(true);
      const response: any = await allMessages(searchTerm, currentPage);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
            ...
            users?.map((user: any, index: number) => (
              <div
                key={user.id}
                className="block relative bg-white h-[500px] dark:bg-gray-800 hover:scale-105 transition-all dark:border-blue-400 dark:border rounded-lg shadow-md p-4"
              >
                <div
                  className="absolute inset-0 bg-cover bg-center opacity-40 hover:opacity-20"
                  style={{
                    padding: "40px",
                    backgroundImage: `url(${
                      imagesArr[index % imagesArr.length].src
                    })`,
                  }}
                ></div>
                <div className="mt-[250px] sm:mt-[250px] p-4 rounded-lg md:mt-[250px] lg:mt-[250px] mx-auto max-w-xl">
                  <Link href={`/user/${user.id}`}>
                    ...
                    <p className="text-sm sm:text-lg font-semibold text-white mb-1">
                      Message:{" "}
                      {user?.message?.length > 60
            ...
            <p className="text-xl dark:text-white text-black font-bold mb-2">
              {" "}
              No message Yet. Be the first to send your wishes by clicking{" "}
